...
void getPrefixSumSolution_WorkEfficient_BCA(int* d_A, int **d_outdata, int length);

ScanWorkEfficient_BCA::ScanWorkEfficient_BCA(int *a, int len): Scan(a, len){}
void ScanWorkEfficient_BCA::getPrefixSum() {
    ...
    timer2.Start();
    getPrefixSumSolution_WorkEfficient_BCA(d_A, &d_preSum, n);
    timer2.Stop();
...
__global__
void scan_WorkEfficient_BCA_finalAdd(int *indata_prefixSumPerBlock, int *indata_d_intermediatePrefixSum, int *outdata_prefixSum, int length) {
...
void getPrefixSumSolution_WorkEfficient_BCA(int* d_A, int **d_outdata, int length) {
    ...
    scan_WorkEfficient_BCA_OneBlock<<<GridSize, BLOCK_SIZE>>>(d_A, d_prefixSumPerBlock, d_intermediateSumPerBlock, length);
    ...
    if (intermediateNum == 1) {
        *d_outdata = d_prefixSumPerBlock;
        ...
        getPrefixSumSolution_WorkEfficient_BCA(d_intermediateSumPerBlock, &d_intermediatePrefixSum, intermediateNum);
        int *d_prefixSum;
        cudaMalloc((void **)&d_prefixSum, length * sizeof(int));
        scan_WorkEfficient_BCA_finalAdd<<<ceil((double)length/(BLOCK_SIZE*2)), BLOCK_SIZE>>>(d_prefixSumPerBlock, d_intermediatePrefixSum, d_prefixSum, length);
        cudaDeviceSynchronize();
